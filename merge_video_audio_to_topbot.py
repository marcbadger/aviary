...
def merge_audio_and_video(audio_file='audio.raw',video_file='video.raw',
    ...
    #print("first_video_time - first_audio_time: %.4fs " % video_delay)
    # bashCommand = "ffmpeg -y -r %f %s -i %s -f %s -ar %f -ac %d %s -i %s %s -c:v copy %s %s" % (
    #filter_complex = "[0:v] crop=in_w/4:in_h/2:in_w*(0/4):0 [topupperleft]; [0:v] crop=in_w/4:in_h/2:in_w*(1/4):0 [topupperright]; [0:v] crop=in_w/4:in_h/2:in_w*(2/4):0 [toplowerleft]; [0:v] crop=in_w/4:in_h/2:in_w*(3/4):0 [toplowerright]; [topupperleft][topupperright][toplowerleft][toplowerright] xstack=inputs=4:layout=0_0|w0_0|0_h0|w0_h0 [topout]; [0:v] crop=in_w/4:in_h/2:in_w*(0/4):in_h/2 [botupperleft]; [0:v] crop=in_w/4:in_h/2:in_w*(1/4):in_h/2 [botupperright]; [0:v] crop=in_w/4:in_h/2:in_w*(2/4):in_h/2 [botlowerleft]; [0:v] crop=in_w/4:in_h/2:in_w*(3/4):in_h/2 [botlowerright]; [botupperleft][botupperright][botlowerleft][botlowerright] xstack=inputs=4:layout=0_0|w0_0|0_h0|w0_h0 [botout]; [1:a:0] pan=stereo|c0<c0|c1<c2 [topaout]; [1:a:0] pan=stereo|c0<c0|c1<c2 [botaout]"
    top_out = output_file + "_top.mp4"
    bot_out = output_file + "_bot.mp4"
    # bashCommand = "ffmpeg -y -r %f %s -i %s -f %s -ar %f -ac %d %s -i %s %s -filter_complex \"%s\" -map \"[topout]\" -c:v %s %s %s -map \"[botout]\" -c:v %s %s %s" % (
    #     video_aux['rate'], delay_video, video_file, 's24le',
    #     audio_aux['rate'], audio_aux['channels'], delay_audio,
    #     audio_file, map_channel, filter_complex, video_format, out_aac, top_out, video_format, out_aac, bot_out)
