...
namespace ffmpeg_image_transport_tools {
  ...
  static std::string
  make_file_name(const std::string &prefix, const ros::Time counter,
                 const std::string &postfix = "") {
    std::stringstream ss;
    ss << prefix << counter << postfix << ".jpg";
    return (ss.str());
  }
  ...
  void DecodeBag::syncCallback(const std::vector<ImageConstPtr> &msgs) {
    ...
    if (writeFrames_ || writeVideo_) {
      ...
      if (writeFrames_ && (frameNum_ % keepRatio_ == 0)) {
        ...
        cv::imwrite(make_file_name(frameBaseName_, msg0->header.stamp), fullImg_);
      }
      frameNum_++;
